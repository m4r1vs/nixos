...
''
  ...
  if [ -z "$random_album" ] || [ "$exit_code" -ne 0 ]; then
    ${import ./nixos-notify.nix pkgs} -e "Failed to query Bandcamp's" "Album of the Day (got \"$random_album\")"
    exit 0
  fi
  ...
  if [ -z "$id" ] || [ "$exit_code" -ne 0 ]; then
    ${import ./nixos-notify.nix pkgs} -e "Failed to search Spotify for" "\"$random_album\""
    exit 0
  fi
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    ${import ./nixos-notify.nix pkgs} -e "Failed to start Playback of" "\"$random_album\""
    exit 0
  fi

  RESPONSE=$(${import ./nixos-notify.nix pkgs} -e --action="open=Open on Bandcamp" "Playing a random Album of the Day:" "\"$random_album\"")

  if [[ "$RESPONSE" == *"open"* ]]; then
    ALBUM_PATH=$(echo "$HTTP" | grep '<a class="title" href="/album-of-the-day/' | shuf -n 1 | awk -F 'href="|">' '{print $3}')
    ${import ./nixos-notify.nix pkgs} -e -t 1000 "$(${pkgs.xdg-utils}/bin/xdg-open "https://daily.bandcamp.com/$ALBUM_PATH")"
  fi
